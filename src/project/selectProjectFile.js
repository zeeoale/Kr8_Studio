...
export async function selectKr8ProjectFile(options = {}) {
  ...
  const initialDirectory = path.win32.resolve(options.initialDirectory || process.cwd());
...
export function normalizeSelectedProjectPath(value) {
  ...
  if (!selected) return '';
  if (path.win32.basename(selected).toLowerCase() !== PROJECT_FILENAME) {
    throw new Error('Select the project.json file inside a .kr8 project folder.');
  }
  return path.win32.resolve(selected);
}
